...
def task(request, pk):
    ...
    return render(request, "taskapp/task.html", {"task": task, "task_type": task_type})
...
def login_form(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            return render(
                request,
                "taskapp/login_register_form.html",
                {"login": True, "error": "Invalid username or password."},
            )
...
def add_task(request, room_id, task_type):
    ...
    if not room.users.filter(id=request.user.id).exists():
        return HttpResponseForbidden(
            "You do not have permission to add tasks to this room."
        )
    ...
    return render(
        ...
        "taskapp/add_task.html",
        {
            "form": form,
            "room": room,
            "task_type": task_type[0 : task_type.find("task")],
        },
    )
